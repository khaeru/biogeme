...
bioArithPrint::bioArithPrint(bioExpressionRepository* rep,
  ...
  : bioExpression(rep, par),theIteratorName(it),theExpressionsId(theTerms), theRandomDraws(NULL) {
...
void bioArithPrint::simulate(patHybridMatrix* varCovar, 
  ...
  patULong nDraws = bioParameters::the()->getValueInt("NbrOfDrawsForSensitivityAnalysis",err)  ;
  if (err != NULL) {
    WARNING(err->describe()) ;
    return ;
  }
  ...
  if (varCovar != NULL) {
    ...
    patReal se =  bioParameters::the()->getValueReal("TolSchnabelEskow",err) ;
    if (err != NULL) {
      WARNING(err->describe()) ;
      return ;
    }
  ...
  if (theIteratorType == ROW) {
	...
	simulatedValues[weightHeader].push_back(w) ;
	...
	for (patULong i = 0 ; i < theHeaders.size() ; ++i) {
	  ...
	  simulatedValues[str.str()].push_back((*(theIter->currentItem()))[j]) ;
	...
	simulatedValues[i->first].push_back(res) ;
	...
	if (varCovar != NULL) {
	  ...
	  for (patULong r = 0 ; r < nDraws ; ++r) { 
	    ...
	    stringstream str ;
	    theAnalysis.addData(i->first,res) ;
	  }
	  ...
	  qa.push_back(alpha) ;
	  qa.push_back(0.5) ;
	  qa.push_back(1-alpha) ;
	  vector<patReal> quantiles = theAnalysis.getQuantiles(i->first,qa,err) ;
	  if (err != NULL) {
	    WARNING(err->describe()) ;
	    return ;
	  }
	  stringstream q1,q2,qmed ;
	  q1 << i->first << "_" << int(alpha*100) ;
	  simulatedValues[patString(q1.str())].push_back(quantiles[0]) ;
	  qmed << i->first << "_median" ;
	  simulatedValues[patString(qmed.str())].push_back(quantiles[1]) ;
	  q2 << i->first << "_" << int(100*(1-alpha)) ;
	  simulatedValues[patString(q2.str())].push_back(quantiles[2]) ;
	  patULong dump = bioParameters::the()->getValueInt("DumpSensitivityAnalysis",err) ;
	  if (err != NULL) {
	    WARNING(err->describe()) ;
	    return ;
	  }
	  if (dump != 0) {
	    theAnalysis.dumpOnFile(err) ;
	  }
	  if (err != NULL) {
	    WARNING(err->describe()) ;
	    return ;
...
map<patString,vector<patReal> >* bioArithPrint::getSimulationResults()  {
